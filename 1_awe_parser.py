# awe_parser from Another EDF Tools, author s.#8549

# edit by demonized:
# 1. filename of extracted sound is {paddedSerial}_{file_name}.hca, where paddedSerial is 0-padded sound id in AWB to ensure the correct order
# 2. Warning instead of crash when AWE file list length != AWB file list length, the game is fine with that. Only sounds found in AWE list will be extracted from AWB
...
def extract(awe_path: str, awb_path: str, extract_path: str = None):
    ...
    with AWBParse(awb_path) as awb:
        ...
        if awe.list_length != awb.list_length:
            print('WARNING: awe or awb file count mismatch')
            # raise FileNotFoundError
        awe.parse()
        awb.parse()
        ...
        padCount = len(str(awe.list_length)) + 1

        for serial, file_name in awe.name_table.items():
            paddedSerial = str(serial).zfill(padCount)
            output_path = output_dir / f'{paddedSerial}_{file_name}.hca'
            print(paddedSerial, file_name)
